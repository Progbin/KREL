...
class _CodeTree:
    def __init__(self, tree, parent=None, seven_chr_def_ancestor=None, seven_chr_note_ancestor=None,
                 use_additional_code_ancestor=None, code_first_ancestor=None, code_also_ancestor=None,
                 classified_elsewhere_ancestor=None):
        ...
        # [MODIFIED] Changed to List to support "; ".join() in user wrapper
        self.code_also = []
        self.code_also_ancestor = code_also_ancestor
        self.in_diseases_classified_elsewhere = []
        self.is_classified_elsewhere = False
        self.classified_elsewhere_ancestor = classified_elsewhere_ancestor
        ...
        new_classified_elsewhere_ancestor = classified_elsewhere_ancestor

        # Inherit is_classified_elsewhere status from ancestor if present
        if classified_elsewhere_ancestor:
            self.is_classified_elsewhere = True
            if classified_elsewhere_ancestor.in_diseases_classified_elsewhere:
                self.in_diseases_classified_elsewhere = classified_elsewhere_ancestor.in_diseases_classified_elsewhere.copy()

        if "id" in tree.attrib:
            self.name = tree.attrib["id"]
        for subtree in tree:
            if subtree.tag == "section" or subtree.tag == "diag":
                self.children.append(_CodeTree(subtree, parent=self,
                                               ...
                                               code_also_ancestor=new_code_also_ancestor,
                                               classified_elsewhere_ancestor=new_classified_elsewhere_ancestor))
                ...
                if subtree.text and subtree.text.strip():
                    self.use_additional_code.append(subtree.text.strip())
                for child in subtree:
                    if child.text and child.text.strip():
                        self.use_additional_code.append(child.text.strip())
                new_use_additional_code_ancestor = self
            elif subtree.tag == "codeFirst":
                if subtree.text and subtree.text.strip():
                    self.code_first.append(subtree.text.strip())
                for child in subtree:
                    if child.text and child.text.strip():
                        self.code_first.append(child.text.strip())
                new_code_first_ancestor = self


            # [MODIFIED] Logic to append to List instead of concatenating String
            elif subtree.tag == "codeAlso":
                # Priority 1: Direct text (most common for codeAlso)
                if subtree.text and subtree.text.strip():
                    self.code_also.append(subtree.text.strip())
                # Priority 2: Children nodes (if any)
                for child in subtree:
                    if child.text and child.text.strip():
                        self.code_also.append(child.text.strip())
                new_code_also_ancestor = self

            elif subtree.tag == "inDiseasesClassifiedElsewhere":
                self.is_classified_elsewhere = True
                if subtree.text and subtree.text.strip():
                    self.in_diseases_classified_elsewhere.append(subtree.text.strip())
                for child in subtree:
                    if child.text and child.text.strip():
                        self.in_diseases_classified_elsewhere.append(child.text.strip())
                new_classified_elsewhere_ancestor = self
        ...
        if self.name not in _code_to_node:
        ...
        if len(self.children) == 0 and (
                self.seven_chr_def != {} or self.seven_chr_def_ancestor != None) and self.type != "extended subcategory":
            ...
            for extension in dictionary:
                if ((extended_name[:3] + extended_name[4:] + extension) in all_confirmed_codes):
                    new_XML = "<diag_ext><name>" + extended_name + extension + "</name><desc>" + self.description + ", " + \
                              dictionary[extension] + "</desc></diag_ext>"
                    self.children.append(_CodeTree(ET.fromstring(new_XML), parent=self,
                                                   seven_chr_def_ancestor=new_seven_chr_def_ancestor,
                                                   seven_chr_note_ancestor=new_seven_chr_note_ancestor,
                                                   use_additional_code_ancestor=new_use_additional_code_ancestor,
                                                   code_first_ancestor=new_code_first_ancestor,
...
def _load_codes(all_codes_file_path: Optional[str] = None, classification_data_file_path: Optional[str] = None) -> None:
    ...
    del all_confirmed_codes


_load_codes()


def change_version(all_codes_file_path: Optional[str] = None,
                   classification_data_file_path: Optional[str] = None) -> None:
    if (all_codes_file_path is None and classification_data_file_path is None) or (
            all_codes_file_path is not None and classification_data_file_path is not None):
        _load_codes(all_codes_file_path=all_codes_file_path,
                    classification_data_file_path=classification_data_file_path)
    else:
...
def is_block(code: str) -> bool:
    code = _add_dot_to_code(code)
    if code in _code_to_node:
        return _code_to_node[code].type == "section" or _code_to_node[code].parent != None and _code_to_node[
            code].parent.name == code  # type: ignore[reportOptionalMemberAccess]
    else:
        return False
...
def is_subcategory(code: str, include_extended_subcategories: bool = True) -> bool:
    code = _add_dot_to_code(code)
    if code in _code_to_node:
        return _code_to_node[code].type == "subcategory" or _code_to_node[
            code].type == "extended subcategory" and include_extended_subcategories
    else:
        return False
...
# --- [MODIFIED] Getter Functions now return LIST[str] to support join() ---

def get_code_also(code: str, search_in_ancestors: bool = False, prioritize_blocks: bool = False) -> list[str]:
    if not is_valid_item(code):
        raise ValueError("The code \"" + code + "\" does not exist.")
    node = _code_to_node[_add_dot_to_code(code)]
    if prioritize_blocks and node.parent != None and node.parent.name == node.name:
        node = node.parent
    res = node.code_also.copy()  # Return copy of list
    if search_in_ancestors and len(res) == 0 and node.code_also_ancestor != None:
        return node.code_also_ancestor.code_also.copy()
    else:
        return res


def is_in_diseases_classified_elsewhere(code: str, search_in_ancestors: bool = False,
                                        prioritize_blocks: bool = False) -> bool:
    if not is_valid_item(code):
        raise ValueError("The code \"" + code + "\" does not exist.")
    node = _code_to_node[_add_dot_to_code(code)]
    if prioritize_blocks and node.parent != None and node.parent.name == node.name:
        node = node.parent
    if node.is_classified_elsewhere:
        return True
    if search_in_ancestors and node.classified_elsewhere_ancestor != None:
        return node.classified_elsewhere_ancestor.is_classified_elsewhere
    return False


def get_in_diseases_classified_elsewhere_text(code: str, search_in_ancestors: bool = False,
                                              prioritize_blocks: bool = False) -> list[str]:
    """
    返回 Manifestation 的文本列表。
    修改后：因为子节点初始化时已自动继承了父节点的数据，所以直接返回自身列表即可。
    """
    if not is_valid_item(code):
        raise ValueError("The code \"" + code + "\" does not exist.")
    node = _code_to_node[_add_dot_to_code(code)]
    ...
    # 直接返回列表的副本，确保 List 结构
    return node.in_diseases_classified_elsewhere.copy()
...
def get_full_data(code: str, search_in_ancestors: bool = False, prioritize_blocks: bool = False) -> str:
    ...
    seven_chr_note = get_seven_chr_note(code, search_in_ancestors=search_in_ancestors,
                                        prioritize_blocks=prioritize_blocks)
    if seven_chr_note != "":
        str = str + "\nseven chr note:\n" + seven_chr_note
    ...
    # --- [MODIFIED] Correctly iterating over lists for full data string ---
    code_also_list = get_code_also(code, search_in_ancestors=search_in_ancestors, prioritize_blocks=prioritize_blocks)
    if code_also_list:
        str = str + "\ncode also:"
        for item in code_also_list:
            str = str + "\n" + item

    is_elsewhere = is_in_diseases_classified_elsewhere(code, search_in_ancestors=search_in_ancestors,
                                                       prioritize_blocks=prioritize_blocks)
    if is_elsewhere:
        str = str + "\nin diseases classified elsewhere: True"
        text_elsewhere_list = get_in_diseases_classified_elsewhere_text(code, search_in_ancestors=search_in_ancestors,
                                                                        prioritize_blocks=prioritize_blocks)
        if text_elsewhere_list:
            # Just append text lines if any
            for item in text_elsewhere_list:
                str = str + "\n" + item
    # -----------------------------------
...
# ... (Rest of the file remains same: get_ancestors, etc.) ...
...
def get_nearest_common_ancestor(a: str, b: str, prioritize_blocks_a: bool = False,
                                prioritize_blocks_b: bool = False) -> str:
